...
import { tokenExpiryState, tokenStateLabelHe, isTokenActionable } from "./token-expiry";
...
describe("tokenExpiryState", () => {
  ...
  it("returns unknown when user_token but no expiry date", () => {
    const b = mkBusiness({ meta_auth_mode: "user_token", meta_access_token_expires_at: null });
    expect(tokenExpiryState(b, NOW)).toEqual({ kind: "unknown" });
  });

  it("returns healthy when >10 days away", () => {
    const b = mkBusiness({ meta_access_token_expires_at: "2026-06-01T00:00:00Z" });
  ...
  it("returns warning at 10 days or less", () => {
    const b = mkBusiness({ meta_access_token_expires_at: "2026-04-28T12:00:00Z" });
    const state = tokenExpiryState(b, NOW);
  ...
  it("returns critical at 3 days or less", () => {
    const b = mkBusiness({ meta_access_token_expires_at: "2026-04-23T12:00:00Z" });
    const state = tokenExpiryState(b, NOW);
  ...
  it("returns critical at exactly 0 days", () => {
    const b = mkBusiness({ meta_access_token_expires_at: "2026-04-22T00:00:00Z" });
    const state = tokenExpiryState(b, NOW);
    expect(state.kind).toBe("critical");
  ...
  it("returns expired when past the expiry date", () => {
    const b = mkBusiness({ meta_access_token_expires_at: "2026-04-15T12:00:00Z" });
    const state = tokenExpiryState(b, NOW);
...
describe("tokenStateLabelHe", () => {
  it("renders Hebrew for each state", () => {
    expect(tokenStateLabelHe({ kind: "no_expiry" })).toContain("System User Token");
    expect(tokenStateLabelHe({ kind: "unknown" })).toContain("לא ידוע");
    expect(tokenStateLabelHe({ kind: "expired", daysAgo: 5 })).toContain("פג לפני 5");
    expect(tokenStateLabelHe({ kind: "critical", daysLeft: 2 })).toContain("בעוד 2");
    expect(tokenStateLabelHe({ kind: "critical", daysLeft: 0 })).toContain("פג היום");
    expect(tokenStateLabelHe({ kind: "warning", daysLeft: 7 })).toContain("בעוד 7");
    expect(tokenStateLabelHe({ kind: "healthy", daysLeft: 50 })).toContain("עוד 50");
  });
});
