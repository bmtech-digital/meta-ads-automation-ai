...
**⚠️ DRY RUN MODE — NO WRITE OPERATIONS ALLOWED ⚠️**

**DO NOT call any MCP write tools.** Specifically, NEVER call:
- `update_adset()` — do NOT change budgets
- `pause_adset()` — do NOT pause ad sets
- `pause_ad()` — do NOT pause ads
- `create_adset()` — do NOT create new ad sets

Instead, output the full list of recommended actions in the table format from Step 10, with clear reasoning for each action. Let the user review and decide whether to enable execution in a future session.

READ-only MCP tools (`get_campaigns`, `get_adsets`, `get_ads`, `get_insights`) are allowed.
